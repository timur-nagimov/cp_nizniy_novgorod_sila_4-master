...
if prompt := st.chat_input("Введите свой запрос"):
    ...
    if preset_answer:
        ...
        st.session_state.messages.append({"role": "assistant", "content": preset_answer})
        ...
        with st.spinner("Обработка запроса"):
            with st.chat_message("assistant"):
                ...
                if isinstance(response, str):
                    ...
                    st.session_state.messages.append({"role": "assistant", "context": response['context'], "content": response["response"].text})
                    log = PromptHistory(question=prompt, rewritten_question=response['rewritten_query'], context=response['context'], answer=response['response'].text)
