...
def get_fusion_retriever(index, vector_top_k=10, bm25_top_k=10, total_top_k=3):
    ...
    bm25_retriever = BM25Retriever.from_defaults(
        docstore=index.docstore, similarity_top_k=bm25_top_k, language='ru', stemmer=Stemmer.Stemmer("russian")
    )
...
class ResponseWithChatHistory(CustomQueryComponent):
    ...
    def _run_component(self, **kwargs) -> Dict[str, Any]:
        ...
        prepared_context, node_context = self._prepare_context(
            nodes, query_str
        )
...
def get_rag_pipeline(retriever):
    ...
    pipeline.add_link(
        "llm", "response_component", dest_key="query_str"
    )
...
def get_response(history_str, query_str):
    ...
    if router_res != '0':
        ...
        to_bd = PipelineDB(
            ...
            context = [inter.get_content() for inter in intermediates['rewrite_retriever'].outputs['output']],
